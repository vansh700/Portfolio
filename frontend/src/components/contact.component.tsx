...
import { Mail, Send, MapPin, Circle } from 'lucide-react';
import { useQuery } from '@tanstack/react-query';
import { fetchProfile } from '@/api/profile.api';
import { sendContact } from '@/api/auth.api';
...
export default function ContactComponent() {
  ...
  const socials = [
    {
      emoji: '🐙',
      label: 'GitHub',
      href: profile?.githubUrl ?? 'https://github.com/vansh700',
      color: '#e2e8f0',
    },
    {
      emoji: '💼',
      label: 'LinkedIn',
      href: profile?.linkedinUrl ?? 'https://www.linkedin.com/in/vansh-soni-85921129a/',
      color: '#0ea5e9',
    },
    {
      emoji: '📧',
      label: 'Email',
      href: `mailto:${profile?.email ?? 'vanshsoni513@gmail.com'}`,
      color: '#6366f1',
    },
    {
      emoji: '🐦',
      label: 'Twitter',
      href: profile?.twitterUrl ?? 'https://twitter.com/vansh700',
      color: '#38bdf8',
    },
  ];
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSending(true);
    try {
      const result = await sendContact(form.name, form.email, form.message);
      setSent(true);
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <div className="glass" style={{ padding: '1.75rem', borderRadius: 18 }}>
              ...
              >
                Open to freelance projects, full-time roles, and exciting collaborations. I usually
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ color: '#cbd5e1', fontSize: '0.875rem' }}>
                    {profile?.email ?? 'vanshsoni513@gmail.com'}
                  </span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <MapPin size={15} color="#6366f1" />
                  <span style={{ color: '#94a3b8', fontSize: '0.875rem' }}>
                    {profile?.location ?? 'India · Open to Remote'}
                  </span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <Circle
              ...
              >
                {socials.map((s) => (
                  ...
                  >
                    ...
                    <span style={{ fontSize: '0.825rem', fontWeight: 600, color: '#cbd5e1' }}>
                      {s.label}
                    </span>
                  </a>
                ))}
        ...
        >
          <p style={{ color: '#334155', fontSize: '0.85rem' }}>
            Built with ❤️ by{' '}
            <span style={{ color: '#6366f1' }}>{profile?.name ?? 'Vansh Soni'}</span> · Next.js +
            TailwindCSS · 2025
          </p>
        </div>
      </div>
